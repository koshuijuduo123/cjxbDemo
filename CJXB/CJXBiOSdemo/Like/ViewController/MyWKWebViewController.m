...
@interface MyWKWebViewController ()<WKUIDelegate,WKNavigationDelegate,selectIndexPathDelegate,UMSocialUIDelegate>
{
    NSTimer *myTimer;
}
...
- (void)selectIndexPathRow:(NSInteger)index{
    ...
    if (index==0) {
        ...
        if (pasteboard.string.length) {
            [MyWKWebViewController showAlertMessageWithMessage:@"复制成功" duration:1.0];
            
        }else{
            [MyWKWebViewController showAlertMessageWithMessage:@"复制失败" duration:1.0];
        }
    }
}
...
-(void)didFinishGetUMSocialDataInViewController:(UMSocialResponseEntity *)response{
    if (response.responseCode==UMSResponseCodeSuccess) {
        [MyWKWebViewController showAlertMessageWithMessage:@"分享成功" duration:1.0];
    }    
}
