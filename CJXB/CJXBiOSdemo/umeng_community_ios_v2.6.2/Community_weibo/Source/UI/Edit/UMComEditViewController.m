...
- (void)createTextView
{
    ...
    self.realTextView.placeholderLabel.text = UMComLocalizedString(@"um_com_micro_edit_realTextViewPlaceholder", @" 新鲜事，由你来发布");
...
//        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(3, 0, self.topicNoticeBgView.frame.size.width, 25)];
    ...
    if ([[[[UMComSession sharedInstance] loginUser] gender] integerValue] == 1) {
        label.text = UMComLocalizedString(@"um_com_micro_edit_addTopicTipForMale", @"wuli哥,添加个话题吧!");
    }else{
...
- (void)dealWhenPostFeedFinish:(NSArray *)responseObject error:(NSError *)error
{
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        ...
        //[realTimeFeedsViewController.tableView reloadData];
        
        //发送通知
        //创建通知
        //NSNotification *notification = [NSNotification notificationWithName:@"UMTASK" object:nil userInfo:nil];
