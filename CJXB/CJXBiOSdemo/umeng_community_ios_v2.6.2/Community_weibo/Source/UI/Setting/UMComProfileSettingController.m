    ...
    if (self.settingCompletion) {
        ...
        [UMComPushRequest updateWithUser:self.userAccount completion:^(NSError *error) {
            ...
            NSDictionary*parSDic = @{@"exec":@"edituserinfo",@"n":weakSelf.userAccount.name,@"sex":weakSelf.userAccount.gender,@"img":weakSelf.userAccount.icon_url};
                //创建通知
                NSNotification *notification = [NSNotification notificationWithName:@"Save" object:nil userInfo:parSDic];
...
- (IBAction)logout:(id)sender {
    ...
    //发送通知
    //创建通知
    NSNotification *notification = [NSNotification notificationWithName:@"Ruturn" object:nil userInfo:nil];
    [[NSNotificationCenter defaultCenter] postNotification:notification];
