...
+ (void)loginWithLoginViewController:(UIViewController *)loginViewController userAccount:(UMComUserAccount *)loginUserAccount response:(id)responseObject error:(NSError *)error loginCompletion:(LoginCompletion)completion
{
    ...
    if ([responseObject isKindOfClass:[UMComUser class]]) {//登录成功则隐藏登录页面
        ...
        [loginViewController dismissViewControllerAnimated:YES completion:^{
            [UMComLoginManager loginSuccessWithUser:loginUser];
            if ([loginUser.registered integerValue] == 0) {//判断是否第一次登录，如果是第一次登陆则走第一次登录逻辑
                
                [loginManager firstTimeLoginHandleLogic:loginManager.currentViewController loginUserAccount:loginUserAccount responseObject:loginUser loginCompletion:completion];
            }else{
                
                //如果不是第一次登陆则直接返回
                SafeCompletionDataAndError(loginManager.loginCompletion, loginUser, nil);
            }
        }];
